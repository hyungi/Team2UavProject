...
public class CameraStreamF {
    ...
    public CameraStreamF(Canvas canvas) throws Exception {
    	this.canvas = canvas;
        this.gc = canvas.getGraphicsContext2D();
    }
    
    //CamPublisher("http://192.168.3.xxx:50005/?action=stream", "tcp://192.168.3.xxx:1883", "/uav/camera");
    public CameraStreamF(String serverURI, String mqttBrokerURI, String topic) throws Exception {
    ...
    //CamPublisher("http://192.168.3.xxx:50005/?action=stream", "tcp://192.168.3.xxx:1883", "/uav/camera");
    public CameraStreamF(String mqttBrokerURI, String topic, Canvas canvas) throws Exception {
        this.mqttBrokerURI = mqttBrokerURI;
        this.topic = topic;
        this.canvas = canvas;
        this.gc = canvas.getGraphicsContext2D();
    ...
    public CameraStreamF(String serverURI, Canvas canvas) {
        this.serverURI = serverURI;
        this.canvas = canvas;
        this.gc = canvas.getGraphicsContext2D();
    }    
                    ...
                    try {
                        ...
                        mqttClient.setCallback(new MqttCallback() {
                            ...
                            public void messageArrived(String topic, MqttMessage message) throws Exception {
            					...
            					mqttClient.publish(Network.uavCameraSubTopicF, nextMessage);
                        ...
                        mqttClient.subscribe(Network.uavCameraPubTopicF);
                        connected = true;
                        mqttClient.publish(Network.uavCameraSubTopicF, new byte[0], 0, true); // MQTT Broker의 영상버퍼 제거
                        MqttMessage nextMessage = new MqttMessage("next".getBytes()); // next Message 제작
                        mqttClient.publish(Network.uavCameraSubTopicF, nextMessage.getPayload(), 0, true); //next Message 발송
                    } catch(Exception e) {
                        try {
